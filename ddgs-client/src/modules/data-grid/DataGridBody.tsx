...
export default function DataGridBody<T extends TableEntity>({ sortedIds, dataGridRows }: DataGridBodyProps<T>) {
    ...
    //const rows = useCallback(() => [...tableData].map((row) => <DadaGridBodyRow key={row.id} rowData={row} />), [tableData]);

    const rows = useMemo(() => sortedIds.map((rowId) => <DadaGridBodyRow key={rowId} rowData={dataGridRows[rowId]} />), [dataGridRows]);
